        ...
        while (psubchild) 
		{
        	...
        	if ((subTagname == CALIB_SENSOR_DPF_FILT_LEVEL_REG_CONF_ENABLE_TAG)	
				&& (tag.isType(XmlTag::TAG_TYPE_DOUBLE))
             	&& (tag.Size() > 0))
    		{
    			int no = ParseUcharArray(tag.Value(), &dpf_profile.FiltLevelRegConf.FiltLevelRegConfEnable, 1);
      			DCT_ASSERT((no == 1));
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUcharArray(tag.Value(), p_FiltLevel, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				dpf_profile.FiltLevelRegConf.ArraySize = no;
				dpf_profile.FiltLevelRegConf.p_FiltLevel = p_FiltLevel;

				for(int i=0; i < no; i++){
					if(p_FiltLevel[i] > 10){
						redirectOut << "filter level only support 10 level," << p_FiltLevel[i] << " > 10 is invalid: " << std::endl;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUcharArray(tag.Value(), p_grn_stage1, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_grn_stage1 = p_grn_stage1;
				for(int i=0; i < no; i++){
					if(p_grn_stage1[i] > 8){
						redirectOut << "grn_stage1 only support 0-8," << p_grn_stage1[i] << " > 8 is invalid: " << std::endl;
						p_grn_stage1[i] = 8;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUcharArray(tag.Value(), p_chr_h_mode, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_chr_h_mode = p_chr_h_mode;
				for(int i=0; i < no; i++){
					if(p_chr_h_mode[i] > 3){
						redirectOut << "chr_h_mode only support 0-3," << p_chr_h_mode[i] << " > 3 is invalid: " << std::endl;
						p_chr_h_mode[i] = 3;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUcharArray(tag.Value(), p_chr_v_mode, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_chr_v_mode = p_chr_v_mode;
				for(int i=0; i < no; i++){
					if(p_chr_v_mode[i] > 3){
						redirectOut << "chr_v_mode only support 0-3," << p_chr_v_mode[i] << " > 3 is invalid: " << std::endl;
						p_chr_v_mode[i] = 3;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_thresh_bl0, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_thresh_bl0 = p_thresh_bl0;
				for(int i=0; i < no; i++){
					if(p_thresh_bl0[i] > 1023){
						redirectOut << "thresh_bl0 only support 0-1023," << p_thresh_bl0[i] << " > 1023 is invalid: " << std::endl;
						p_thresh_bl0[i] = 1023;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_thresh_bl1, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_thresh_bl1 = p_thresh_bl1;
				for(int i=0; i < no; i++){
					if(p_thresh_bl1[i] > 1023){
						redirectOut << "thresh_bl1 only support 0-1023," << p_thresh_bl1[i] << " > 1023 is invalid: " << std::endl;
						p_thresh_bl1[i] = 1023;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_fac_bl0, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_fac_bl0 = p_fac_bl0;
				for(int i=0; i < no; i++){
					if(p_fac_bl0[i] > 63){
						redirectOut << "fac_bl0 only support 0-63," << p_fac_bl0[i] << " > 63 is invalid: " << std::endl;
						p_fac_bl0[i] = 63;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_fac_bl1, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_fac_bl1 = p_fac_bl1;
				for(int i=0; i < no; i++){
					if(p_fac_bl1[i] > 63){
						redirectOut << "fac_bl1 only support 0-63," << p_fac_bl1[i] << " > 63 is invalid: " << std::endl;
						p_fac_bl1[i] = 63;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_thresh_sh0, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_thresh_sh0 = p_thresh_sh0;
				for(int i=0; i < no; i++){
					if(p_thresh_sh0[i] > 1023){
						redirectOut << "thresh_sh0 only support 0-1023," << p_thresh_sh0[i] << " > 1023 is invalid: " << std::endl;
						p_thresh_sh0[i] = 1023;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_thresh_sh1, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_thresh_sh1 = p_thresh_sh1;
				for(int i=0; i < no; i++){
					if(p_thresh_sh1[i] > 1023){
						redirectOut << "thresh_sh1 only support 0-1023," << p_thresh_sh1[i] << " > 1023 is invalid: " << std::endl;
						p_thresh_sh1[i] = 1023;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_fac_sh0, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_fac_sh0 = p_fac_sh0;
				for(int i=0; i < no; i++){
					if(p_fac_sh0[i] > 63){
						redirectOut << "fac_sh0 only support 0-63," << p_fac_sh0[i] << " > 63 is invalid: " << std::endl;
						p_fac_sh0[i] = 63;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_fac_sh1, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_fac_sh1 = p_fac_sh1;
				for(int i=0; i < no; i++){
					if(p_fac_sh1[i] > 63){
						redirectOut << "fac_sh1 only support 0-63," << p_fac_sh1[i] << " > 63 is invalid: " << std::endl;
						p_fac_sh1[i] = 63;
					}
				}
             	...
             	&& (tag.Size() > 0))
    		{
				...
				int no = ParseUintArray(tag.Value(), p_fac_mid, tag.Size());
          		DCT_ASSERT((no == tag.Size()));
				DCT_ASSERT((no == dpf_profile.FiltLevelRegConf.ArraySize));
				dpf_profile.FiltLevelRegConf.p_fac_mid = p_fac_mid;
				for(int i=0; i < no; i++){
					if(p_fac_mid[i] > 63){
						redirectOut << "fac_mid only support 0-63," << p_fac_mid[i] << " > 63 is invalid: " << std::endl;
						p_fac_mid[i] = 63;
					}
				}
			}else {
...
#endif
	if(tagname == CALIB_SENSOR_DSP_3DNR_SETTING_TAG) {
