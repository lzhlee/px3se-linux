...
(
  ...
  if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
	...
	if(pDpfProfile_t != NULL && pDpfProfile_t->FiltLevelRegConf.FiltLevelRegConfEnable == 1) {
	  for(int i=0; i < pDpfProfile_t->FiltLevelRegConf.ArraySize; i++) {
	  	#if 0
	  	 ALOGE("%s: oyyf filter[%d] %d %d %d %d !", __func__, 
		  	i, pDpfProfile_t->FiltLevelRegConf.p_fac_sh0[i], pDpfProfile_t->FiltLevelRegConf.p_fac_sh1[i],
		  	pDpfProfile_t->FiltLevelRegConf.p_fac_mid[i]);
		#endif
		
		if(flt_cfg->denoise_level == pDpfProfile_t->FiltLevelRegConf.p_FiltLevel[i]){
		  flt_result->fac_bl0 = pDpfProfile_t->FiltLevelRegConf.p_fac_bl0[i];
          flt_result->fac_bl1 = pDpfProfile_t->FiltLevelRegConf.p_fac_bl1[i];
          flt_result->thresh_bl0 = pDpfProfile_t->FiltLevelRegConf.p_thresh_bl0[i];
          flt_result->thresh_bl1 = pDpfProfile_t->FiltLevelRegConf.p_thresh_bl1[i];
          flt_result->chr_h_mode = pDpfProfile_t->FiltLevelRegConf.p_chr_h_mode[i];
          flt_result->chr_v_mode = pDpfProfile_t->FiltLevelRegConf.p_chr_v_mode[i];
          flt_result->grn_stage1 = pDpfProfile_t->FiltLevelRegConf.p_grn_stage1[i];
		}

		if(flt_cfg->sharp_level == pDpfProfile_t->FiltLevelRegConf.p_FiltLevel[i]){
		  flt_result->thresh_sh0 = pDpfProfile_t->FiltLevelRegConf.p_thresh_sh0[i];
          flt_result->thresh_sh1 = pDpfProfile_t->FiltLevelRegConf.p_thresh_sh1[i];
          flt_result->fac_sh0 = pDpfProfile_t->FiltLevelRegConf.p_fac_sh0[i];
          flt_result->fac_sh1 = pDpfProfile_t->FiltLevelRegConf.p_fac_sh1[i];
          flt_result->fac_mid = pDpfProfile_t->FiltLevelRegConf.p_fac_mid[i]; 
		}	
	  }
   }
	#if 0
	ALOGE("%s:flt_cfg->sharp_level %d %d %d %d %p!", __func__, 
		  	flt_cfg->sharp_level, flt_result->fac_sh0, flt_result->fac_sh1,
		  	flt_result->fac_mid, pDpfProfile_t);
	#endif
...
(
  ...
  if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
	...
	if(goc_cfg != NULL){
		...
		if(result != RET_SUCCESS){
			ALOGD("fail to get no GocProfile, ret: %d", result);
		}else{
			if(no_goc > 0){
...
(
  ...
  if (enable_mode == HAL_ISP_ACTIVE_FALSE) {
    ...
    ALOGD("%s:set  cproc use_case %d, ct=%f,hue=%f,bt=%d,st=%f", __func__,
          cproc_cfg->use_case,
      ...
      if (cproc_cfg) {
        ...
        if (cproc_data) {
		  ...
		  if (cproc_cfg->cproc.hue < 0.0f)
			...
			cproc_result->hue = (uint8_t)((cproc_cfg->cproc.hue) * 128.0f / 90.0f);
#if 0
          ALOGD("%s:set xml cproc use_case %d, ct=%f,hue=%f,bt=%d,st=%f", __func__,
                cproc_cfg->use_case,
