...
#include "drmDsp.h"
#include "../HAL/include/DrmCameraBuffer.h"
#endif
...
public:
    ...
    bool processFrame(shared_ptr<BufferBase> inBuf,
        ...
        static int count = 0;
        if (count++ % 100 == 0)
            printf("nv12disp %d, %dx%d\n", count, inBuf->getWidth(), inBuf->getHeight());
        if (inBuf.get()) {
 ...
 public:
  ...
  virtual bool bufferReady(weak_ptr<BufferBase> buffer, int status) {
    ...
    if (spCamBuf.get()) {
      ...
      char fname[50] = {0};
      static int frames = 0;
      int size = spCamBuf->getWidth() * spCamBuf->getHeight() * 3 / 2;
      snprintf(fname, 30, "/tmp/yuv_%dx%d.bin", spCamBuf->getWidth(), spCamBuf->getHeight());
      frames++;
      if ((frames > 30) && (frames < 100)) {
        FILE* yuv_file =  fopen(fname, "a+");
        if (yuv_file) {
          fwrite(spCamBuf->getVirtAddr(),size, 1, yuv_file);
          ALOGD("write 0x%x bytes to file!", size);
          fclose(yuv_file);
        } else
