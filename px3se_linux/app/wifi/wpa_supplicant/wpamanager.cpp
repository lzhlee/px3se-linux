...
int wpaManager::openCtrlConnection(const char *ifname)
{
    ...
    qDebug()<<"ctrl_iface:"<<ctrl_iface;
qDebug()<<"ctrl_iface_dir:"<<ctrl_iface_dir;
    if (ctrl_iface == NULL) {
#ifdef CONFIG_NATIVE_WINDOWS
    ...
    qDebug()<<"Trying to connect to '"<<cfile<<"'";
...
void wpaManager::selectAdapter(const QString & sel)
{
    if (openCtrlConnection(sel.toLocal8Bit().constData()) < 0)
        qDebug()<<"Failed to open control connection to "<< " wpa_supplicant.selectAdapter";
    updateStatus();
    updateNetworks();
}
        ...
        if (pos) {
            *pos++ = '\0';
            if (strcmp(start, "bssid") == 0) {
                ...
                ipaddr_updated = true;
                qDebug()<<"ip_address:"<<pos;
                //wifiWid->m_tabCurrentStatus->textIPAddress->setText(pos);
            } else if (strcmp(start, "wpa_state") == 0) {
                status_updated = true;
                qDebug()<<"wpa_state:"<<wpaStateTranslate(pos);
                emit statusChange(wpaStateTranslate(pos));
...
void wpaManager::signalMeterUpdate()
{
    ...
    if ((rssi = strstr(reply, "AVG_RSSI=")) != NULL)
        ...
        qDebug()<<"Failed to get RSSI value";
        return;
    }

    qDebug()<<"RSSI value: "<< rssi_value;
...
void wpaManager::updateScanResult()
{
    ...
    while (true) {
        ...
        qDebug()<<"ssid:"<<ssid<<" bssid:"<<bssid<<" flags:"<< flags;
        //wifiWid->m_tabScanResult->insertIntoTable(ssid,bssid,signal,flags);
        emit insertNetwork(ssid,bssid,flags,signal);
        if (bssid.isEmpty())
            break;
    }
}
...
void wpaManager::enableNetwork(const QString &sel)
{
    ...
    if (cmd.contains(QRegExp("^\\d+:")))
        ...
        qDebug()<<"Invalid editNetwork "<<cmd.toLocal8Bit().constData();
        return;
    }
...
void wpaManager::disableNetwork(const QString &sel)
{
    ...
    if (cmd.contains(QRegExp("^\\d+:")))
        ...
        qDebug()<<"Invalid editNetwork '%s'"<<cmd.toLocal8Bit().constData();
        return;
    }
...
int wpaManager::getNetworkDisabled(const QString &sel)
{
    ...
    int pos = cmd.indexOf(':');
    if (pos < 0) {
        qDebug()<<"Invalid getNetworkDisabled '%s'"<<cmd.toLocal8Bit().constData();
        return -1;
    }
    cmd.truncate(pos);
...
void wpaManager::saveConfig()
{
    ...
    if (str_match(buf, "FAIL"))
        ...
        qDebug()<<"The current configuration was saved.";
