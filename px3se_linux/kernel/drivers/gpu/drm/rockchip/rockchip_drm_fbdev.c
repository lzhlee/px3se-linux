				     ...
				     struct drm_fb_helper_surface_size *sizes)
{
	...
	if (!uboot_logo_on) {
		...
		uboot_logo_size /= 2; /* only need uboot logo, so div 2 */
		private->logo_handle = ion_alloc(private->ion_client, uboot_logo_size,
					   0, ION_HEAP(ION_CMA_HEAP_ID), 0);
		screen_base = ion_map_kernel(private->ion_client, private->logo_handle);
		ion_phys(private->ion_client, private->logo_handle, &phy_addr, &uboot_logo_size);
		rk_drm_fb_show_kernel_logo(phy_addr, screen_base);
	}
