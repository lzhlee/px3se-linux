				  ...
				  struct rk_screen *dst_screen, bool enable)
{
	...
	if (!enable) {
		rk312x_lcdc_blank(dev_drv, 0, FB_BLANK_NORMAL);
		spin_lock(&lcdc_dev->reg_lock);
		...
		rk312x_lcdc_blank(dev_drv, 0, FB_BLANK_UNBLANK);
...
static int rk312x_lcdc_backlight_close(struct rk_lcdc_driver *dev_drv,
				       int enable)
{
	struct lcdc_device *lcdc_dev =
		container_of(dev_drv, struct lcdc_device, driver);
...
static struct rk_lcdc_drv_ops lcdc_drv_ops = {
	...
	.backlight_close = rk312x_lcdc_backlight_close,
